    ...
    (0, _createClass3.default)(BotApp, [{
        ...
        value: function _init() {
            ...
            if (this.isInApp()) {
                ...
                window.addEventListener('message', function (event) {
                    if (event.origin === _this._postMessageTarget) {
                        var data = event.data;
                ...
                window.addEventListener('popstate', function (event) {
                    if (!event.state) {
                        _this._deleteOrderIframe();
                        _this.oAuthIframeDOM.contentWindow.postMessage({
                            type: 'ship',
                            data: (0, _extends3.default)({}, _this._buyData)
                        }, _this._postMessageTarget);
                    }
                });
            }
        }
        ...
        value: function _createOAuthIframe() {
            if (this.oAuthIframeDOM) {
                ...
                var styleTxt = 'width: 100%;' + 'display: none;' + 'height: 100vh;' + 'position: fixed;' + 'left: 0;' + 'bottom: 0;' + 'z-index: 100;' + 'border: none;' + 'margin: 0;' + 'padding: none;';
                iframe.style.cssText = styleTxt;
                document.body.appendChild(iframe);
            }
        }
    }, {
        key: '_createOrderIframe',
        value: function _createOrderIframe(src) {
            if (this.orderIframeDOM) {
                this.orderIframeDOM.src = src;
                document.body.appendChild(this.orderIframeDOM);
            } else {
                var iframe = this.orderIframeDOM = document.createElement('iframe');
                iframe.id = 'dueros_order_iframe';
                iframe.src = src;
                iframe.frameborder = 'no';
                // iframe.scrolling ='no';
                iframe.allowtransparency = 'yes';
                iframe.style.cssText = 'width: 100%;' + 'background-color: #fff;' + 'height: 100vh;' + 'position: fixed;' + 'left: 0;' + 'bottom: 0;' + 'z-index: 100;' + 'border: none;' + 'margin: 0;' + 'padding: none;';
                document.body.appendChild(iframe);
            }
        }
    }, {
        ...
        key: '_routeOrderIframe',
        value: function _routeOrderIframe() {
            var stateObj = {
                showOrder: true
            };
        ...
        key: '_deleteOrderIframe',
        value: function _deleteOrderIframe() {
        ...
        value: function requireBuy(data, cb) {
            if (this.isInApp()) {
                ...
                var iframeUrl = baseUrl += this._encodeQueryData(postData);
